...
from utils.constants import (
    ...
    SUBSTATS_POOL, RARITY_SCORE, get_gradient_color, get_substat_value, get_padded_kor, parse_kwargs
)
...
def load_data():
    try:
        with open('configs/EggSummonConfig.json', 'r', encoding='utf-8') as f:
            summon_config = json.load(f)
        with open('configs/PetLibrary.json', 'r', encoding='utf-8') as f:
            pet_library = json.load(f)
        return summon_config, pet_library
    except FileNotFoundError:
        print("에러: configs 폴더에 펫용 JSON 파일이 없습니다.")
        return None, None
...
def do_summon(state, args, levels_config, pets_by_rarity, pet_library):
    ...
    p_state = state["Pet"]
    current_seed, current_level, current_count, extra_chance_pct = p_state["seed"], p_state["level"], p_state["count"], p_state["chance"]
    is_ascended = p_state.get("is_ascended", False)
    
    start_idx = len(p_state["history"])
    ...
    for _ in range(times):
        ...
        while current_pull_idx < total_target:
            ...
            chosen_pet = available_pets[pcg_summon.next_int(len(available_pets))]
            ...
            res = {
                ...
                "pet_type": pet_library[chosen_pet]["Type"], "perfection": (f64_sum / num_substats) * 100.0,
                "stats": acquired_stats, "detailed_stats": detailed_stats,
                "is_bonus": is_bonus_pull, "seed": current_seed, "index": len(p_state["history"]) + 1
            }
            p_state["history"].append(res)
    ...
    p_state["seed"], p_state["level"], p_state["count"] = current_seed, current_level, current_count

    print(f"\n[ {amount}회 × {times}번 소환 결과: 총 {total_acquired}마리 ({amount * times} + 보너스 {total_bonus_count}) ]\n┌" + "─"*8 + "┬" + "─"*10 + "┬" + "─"*10 + "┬" + "─"*27 + "┬" + "─"*27 + "┬" + "─"*10 + "┬" + "─"*8 + "┐\n│  순번  │   티어   │   타입   │           스탯 1          │           스탯 2          │  완성도  │ 보너스 │\n├" + "─"*8 + "┼" + "─"*10 + "┼" + "─"*10 + "┼" + "─"*27 + "┼" + "─"*27 + "┼" + "─"*10 + "┼" + "─"*8 + "┤")
    for res in p_state["history"][start_idx:]:
        colored_r = f"{RARITY_COLOR[res['rarity_name']]}{get_padded_kor(RARITY_KOR[res['rarity_name']], 8)}{COLOR_RESET}"
        colored_t = f"{TYPE_COLOR.get(res['pet_type'], '')}{get_padded_kor(res['pet_type'], 8)}{COLOR_RESET}"
        print(f"│ {res['index']:>6} │ {colored_r} │ {colored_t} │{format_stat_with_color(res['detailed_stats'], 0, 27)}│{format_stat_with_color(res['detailed_stats'], 1, 27)}│ {get_gradient_color(res['perfection'])}{res['perfection']:>6.2f}%{COLOR_RESET}  │{'   🍀   ' if res['is_bonus'] else '        '}│")
    print("└" + "─"*8 + "┴" + "─"*10 + "┴" + "─"*10 + "┴" + "─"*27 + "┴" + "─"*27 + "┴" + "─"*10 + "┴" + "─"*8 + "┘")
...
def do_search(state, args):
    ...
    if not target_rarity: return print(f"'{target_name}' 티어 펫을 찾을 수 없습니다.")

    filtered = []
    for res in state["Pet"]["history"]:
    ...
    if not filtered: return print(f"조건에 맞는 펫이 없습니다.")
    filtered.sort(key=lambda x: x['perfection'], reverse=True)
    
    print(f"\n[ {RARITY_COLOR[target_rarity]}{RARITY_KOR[target_rarity].strip()}({target_rarity}){COLOR_RESET} 이상 티어 펫 검색 결과 (조건 충족) ]\n┌" + "─"*8 + "┬" + "─"*10 + "┬" + "─"*10 + "┬" + "─"*27 + "┬" + "─"*27 + "┬" + "─"*10 + "┐\n│  순번  │   티어   │   타입   │           스탯 1          │           스탯 2          │  완성도  │\n├" + "─"*8 + "┼" + "─"*10 + "┼" + "─"*10 + "┼" + "─"*27 + "┼" + "─"*27 + "┼" + "─"*10 + "┤")
    for res in filtered:
        colored_r = f"{RARITY_COLOR[res['rarity_name']]}{get_padded_kor(RARITY_KOR[res['rarity_name']], 8)}{COLOR_RESET}"
        colored_t = f"{TYPE_COLOR.get(res['pet_type'], '')}{get_padded_kor(res['pet_type'], 8)}{COLOR_RESET}"
        
        print(f"│ {res['index']:>6} │ {colored_r} │ {colored_t} │{format_stat_with_color(res['detailed_stats'], 0, 27)}│{format_stat_with_color(res['detailed_stats'], 1, 27)}│ {get_gradient_color(res['perfection'])}{res['perfection']:>6.2f}%{COLOR_RESET}  │")
    print("└" + "─"*8 + "┴" + "─"*10 + "┴" + "─"*10 + "┴" + "─"*27 + "┴" + "─"*27 + "┴" + "─"*10 + "┘")


# =====================================================================
# 🧠 완전 100% 최적해 DP 
# =====================================================================
def do_optimize(state, args, levels_config, pets_by_rarity, pet_library):
    ...
    p_state = state["Pet"]
    start_seed = p_state["seed"]
    start_level = p_state["level"]
    start_count = p_state["count"]
    extra_chance_pct = p_state["chance"]
    is_ascended = p_state.get("is_ascended", False)
    ...
    def eval_seed(seed, lvl_data, is_base):
        ...
        pcg.next_int(len(pets_by_rarity[r_val]))
        pcg.next_guid()
    ...
    if mode_str == 'stat':
        ...
        for cost in path:
            ...
            while curr_pull < total_target:
                ...
                chosen_pet = pets_by_rarity[r_val][pcg_summon.next_int(len(pets_by_rarity[r_val]))]
                pcg_summon.next_guid()
        ...
        if target_results:
            print(f"\n[ 획득 예상 타겟 아이템 목록 ]")
            print("┌" + "─"*8 + "┬" + "─"*10 + "┬" + "─"*10 + "┬" + "─"*27 + "┬" + "─"*27 + "┬" + "─"*10 + "┬" + "─"*8 + "┐")
            print("│  순번  │   티어   │   타입   │           스탯 1          │           스탯 2          │  완성도  │ 보너스 │")
            print("├" + "─"*8 + "┼" + "─"*10 + "┼" + "─"*10 + "┼" + "─"*27 + "┼" + "─"*27 + "┼" + "─"*10 + "┼" + "─"*8 + "┤")
...
def handle_command(state, cmd, args):
    ...
    for key, pdata in sorted(data[1].items(), key=lambda x: x[1]["PetId"]["Id"]):
        pets_by_rarity[pdata["PetId"]["Rarity"]].append(key)
            
    if cmd == "summon":
        do_summon(state, args, data[0]["Levels"], pets_by_rarity, data[1])
    elif cmd == "summary": do_summary(state)
    elif cmd == "search": do_search(state, args)
    elif cmd == "optimize":
        do_optimize(state, args, data[0]["Levels"], pets_by_rarity, data[1])
    else: print(f"[Pet] 지원하지 않는 명령어입니다: {cmd}")