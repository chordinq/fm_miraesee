...
from core.metaplaymath.fd6 import fd6_from_f64_raw, fd6_less_than_raw, fd6_to_double

T = TypeVar("T")


class RandomPCG:
	"""IL: Metaplay.Core.RandomPCG."""
	...
	def next_f64_bits(self) -> int:
		"""IL: NextF64 — PCG32 output stored as F64.Raw."""
		return self._next_pcg32()

	def next_f64_raw(self) -> int:
		"""IL: RandomPCG.NextF64() → F64 struct (raw uint32)."""
		return self.next_f64_bits()

	def next_f64(self) -> float:
		"""IL: F64.get_Double(NextF64())."""
		return f64_from_raw(self.next_f64_raw())

	def next_f64_in_range(self, min_value: float, max_value: float) -> float:
		"""IL: RandomPCG.NextF64InRange — F64 fixed-point min + roll * (max - min)."""
		return f64_from_raw(
			f64_next_f64_in_range_raw(
				f64_to_raw(min_value),
				f64_to_raw(max_value),
				self.next_f64_raw(),
			)
		)

	def next_f64_in_range_raw(self, min_raw: int, max_raw: int) -> int:
		return f64_next_f64_in_range_raw(min_raw, max_raw, self.next_f64_raw())

	def next_fixed_d6_raw(self) -> int:
		"""IL: RandomPCG.NextFixedD6() → FD6.FromF64(NextF64())."""
		return fd6_from_f64_raw(self.next_f64_raw())

	def next_fixed_d6(self) -> float:
		"""FD6 semantic double in [0, 1) from NextFixedD6."""
		return fd6_to_double(self.next_fixed_d6_raw())

	def f64_less_than_accumulated(self, roll_raw: int, accumulated_raw: int) -> bool:
		"""IL: F64.op_LessThan for cumulative chance tables (RollAge, RollRarity)."""
		return f64_less_than_raw(roll_raw, accumulated_raw)

	def compare_fixed_d6_less_than(self, chance_raw: int) -> bool:
		"""IL: NextFixedD6() < chance for RollStat."""
		return fd6_less_than_raw(self.next_fixed_d6_raw(), chance_raw)
